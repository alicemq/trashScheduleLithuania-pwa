...
// Selected container IDs (wasteObjectId); comma-separated. If present, only these are included.
$selectedWasteObjectIds = array_filter(array_map('trim', explode(',', $_GET['w'] ?? '')));
...
if ($contracts && isset($contracts['data'])) { // Only proceed if contracts data is valid
    foreach ($contracts['data'] as $contract) {
        ...
        // If selection was provided, include only selected containers
        if (!empty($selectedWasteObjectIds) && !in_array((string)$contract['wasteObjectId'], $selectedWasteObjectIds, true)) {
            continue;
        }
